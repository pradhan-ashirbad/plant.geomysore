...
function _toIsoDate(v) {
  ...
  if (typeof v === 'number') {
    // Google Sheets serial date — pure UTC arithmetic since a serial number
    // has no timezone of its own (mixing in a local-time epoch here would
    // shift the result a day whenever the process runs ahead of UTC).
    const msPerDay = 86400000;
    const epoch = Date.UTC(1899, 11, 30);
    return new Date(epoch + v * msPerDay).toISOString().slice(0, 10);
  }
  return String(v);
}
...
function _computeAutoCalc(sheetName, colMap, rowArray) {
  ...
  if (sheetName === SH.FILTER) {
    const cake  = get('Cake Wt');
    const moist = get('Moisture');
    if (!isNaN(cake) && !isNaN(moist)) set('Dry Wt', +(cake * (1 - moist / 100)).toFixed(2));
    const dry = get('Dry Wt');
    const au  = get('Au');
...
function _recomputeDisplayAutoCalc(sheetName, rowObj) {
  ...
  if (sheetName === SH.FILTER) {
    if (isBlank(rowObj['Dry Wt'])) {
      const cake  = num(rowObj['Cake Wt']);
      const moist = num(rowObj['Moisture']);
      if (!isNaN(cake) && !isNaN(moist)) rowObj['Dry Wt'] = +(cake * (1 - moist / 100)).toFixed(2);
    }
    if (isBlank(rowObj['Au (g)'])) {
      const dry = num(rowObj['Dry Wt']);
