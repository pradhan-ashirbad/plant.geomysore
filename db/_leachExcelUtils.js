...
function dateToISO(v) {
  if (v instanceof Date && !isNaN(v)) {
    // Some source workbooks carry a few hours/seconds of drift on their date
    // serials from an upstream tool that built them off a local-midnight
    // Date().getTime() instead of a clean UTC day count (e.g. IST midnight
    // encoded as ~18:30 the previous UTC day). Round to the nearest UTC day
    // so that drift doesn't roll the date back — a no-op for clean serials.
