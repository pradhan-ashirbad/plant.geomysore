    ...
    return `
      <div class="sec-card ${statusCls}" style="--sec-color:${color}" onclick="openSection('${key}')">
        <div class="sec-card-header">
          <div><div class="sec-card-label">${sec.label}</div></div>
          ${pillHtml(sec.status)}
        </div>
...
function renderCrushing(data) {
  ...
  return `
    <div class="kpi-row-3">
      <div class="kpi-tile"><div class="kpi-label">Running Hours</div><div class="kpi-val">${fmt(totalHrs,1)}</div><div class="kpi-unit">hrs</div></div>
      <div class="kpi-tile"><div class="kpi-label">Production</div><div class="kpi-val gold">${fmt(totalProd,0)}</div><div class="kpi-unit">tonnes</div></div>
      <div class="kpi-tile"><div class="kpi-label">Avg TPH</div><div class="kpi-val">${fmt(avgTph,1)}</div><div class="kpi-unit">t/hr</div></div>
    </div>
    <div class="chart-wrap"><div class="chart-title">Production Trend</div><div class="chart-canvas-wrap"><canvas id="chart-crushing-prod"></canvas></div></div>
    ${chemStrip(data.chemicals)}${stoppagesHtml(data.stoppages)}${genericTableHtml(data)}`;
}
...
function renderMilling(data) {
  ...
  return `
    <div class="kpi-row-3">
      <div class="kpi-tile"><div class="kpi-label">Running Hours</div><div class="kpi-val">${fmt(totalHrs,1)}</div><div class="kpi-unit">hrs</div></div>
      <div class="kpi-tile"><div class="kpi-label">Production</div><div class="kpi-val gold">${fmt(totalProd,0)}</div><div class="kpi-unit">tonnes</div></div>
      <div class="kpi-tile"><div class="kpi-label">Avg Feed Grade</div><div class="kpi-val">${fmt(avgFG,2)}</div><div class="kpi-unit">g/t</div></div>
    </div>
    <div class="chart-wrap"><div class="chart-title">Milling Trend</div><div class="chart-canvas-wrap"><canvas id="chart-milling-prod"></canvas></div></div>
    ${chemStrip(data.chemicals)}${stoppagesHtml(data.stoppages)}${genericTableHtml(data)}`;
}
...
function renderLeaching(data) {
  return `
    <div class="section-block">
      ...
      <div id="leach-heatmap-wrap">${renderLeachingHeatmap(data, 'nacn')}</div>
    </div>
    <div class="chart-wrap"><div class="chart-title">Leaching Trend — LT9 NaCN</div><div class="chart-canvas-wrap"><canvas id="chart-leach-main"></canvas></div></div>
...
function renderFilterPress(data) {
  ...
  return `
    <div class="kpi-row-2">
      <div class="kpi-tile" style="--kpi-color:#2471A3"><div class="kpi-label">AM Shift — Avg Au</div><div class="kpi-val">${avgAu(amRows) !== null ? fmt(avgAu(amRows),2) : '—'}</div><div class="kpi-unit">ppm · ${amRows.length} readings</div></div>
      <div class="kpi-tile" style="--kpi-color:#C0392B"><div class="kpi-label">PM Shift — Avg Au</div><div class="kpi-val">${avgAu(pmRows) !== null ? fmt(avgAu(pmRows),2) : '—'}</div><div class="kpi-unit">ppm · ${pmRows.length} readings</div></div>
    </div>
    <div class="chart-wrap"><div class="chart-title">Au by Shift</div><div class="chart-canvas-wrap"><canvas id="chart-fp-au"></canvas></div></div>
    ${stoppagesHtml(data.stoppages)}${genericTableHtml(data)}`;
}
...
function renderGold(data) {
  ...
  return `
    <div class="kpi-row-2">
      <div class="kpi-tile" style="--kpi-color:var(--gold)"><div class="kpi-label">Total Dore Mass</div><div class="kpi-val gold">${fmt(totalMass,1)}</div><div class="kpi-unit">g</div></div>
      <div class="kpi-tile" style="--kpi-color:var(--gold)"><div class="kpi-label">Total Au Content</div><div class="kpi-val gold">${fmt(totalAu,1)}</div><div class="kpi-unit">g</div></div>
    </div>
    <div class="chart-wrap"><div class="chart-title">Gold Production Trend</div><div class="chart-canvas-wrap"><canvas id="chart-gold-prod"></canvas></div></div>
    ${genericTableHtml(data)}`;
}
  ...
  return `
    <div class="section-block">
      ...
      <div class="tbl-wrap"><table class="dtbl">
        <thead><tr><th>Date</th><th>Section</th><th>Stop</th><th>Resume</th><th>Hours</th><th>Dept</th><th>Reason</th></tr></thead>
        <tbody>${stoppages.map(s => `<tr>
          <td>${s.date||''}</td><td>${s.section||''}</td><td>${s.stop||''}</td><td>${s.start||''}</td>
          <td class="td-val">${fmt(s.hrs,2)}</td><td>${s.dept||''}</td><td>${s.reason||''}</td>
        </tr>`).join('')}</tbody>
      </table></div>
    </div>`;
}
  ...
  STATE.charts[canvasId] = new Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        ...
        y: { beginAtZero: false, grid: { color: 'rgba(0,0,0,.05)' }, ticks: { font: { size: 10 } }, ...(scales.y || {}) },
        ...(scales.y2 ? { y2: { position: 'right', beginAtZero: false, grid: { display: false }, ticks: { font: { size: 10 } }, ...scales.y2 } } : {}),
      },
    },
  });
}
...
function populateEntrySheets() {
  ...
  const sheetsByRole = {
    supervisor: ['Crushing','Milling','Chemical Consumption','Leaching Tanks','Slurry Samples',
  ...
  (sheetsByRole[STATE.role] || []).forEach(s => {
...
async function loadEntryForm() {
  ...
  if (sheet === 'Leaching Tanks') {
    timeRow.style.display = '';
    timeLabel.textContent = 'Reading Time';
    timeSelect.innerHTML = ['03:00','07:00','11:00','15:00','19:00','23:00'].map(t => `<option value="${t}">${t}</option>`).join('');
  } else if (['Filter Press','Cyclone'].includes(sheet)) {
    timeRow.style.display = '';
    timeLabel.textContent = 'Shift';
    timeSelect.innerHTML = ['AM (00:00-12:00)','PM (12:00-00:00)'].map(t => `<option value="${t}">${t}</option>`).join('');
  } else {
...
function paramInputHtml(p) {
  const id = 'param_' + p.key.replace(/[^a-zA-Z0-9]/g, '_');
  let inputHtml = '';
  if (p.autoCalc) {
    inputHtml = `<div class="param-auto-note">Calculated automatically</div>`;
  } else if (p.isOverflow) {
    inputHtml = `<select id="${id}" name="${p.key}"><option value="">—</option><option>No</option><option>Yes</option></select>`;
  } else if (p.isSelect && p.options) {
    inputHtml = `<select id="${id}" name="${p.key}"><option value="">—</option>${p.options.map(o=>`<option>${o}</option>`).join('')}</select>`;
  } else if (p.isTime) {
...
async function loadTargets() {
  ...
  if (!data || data.error) { content.innerHTML = `<div class="nodata">${data ? data.error : 'Error'}</div>`; return; }
  if (!data.length) { content.innerHTML = '<div class="nodata">No targets set yet.</div>'; return; }
  const byMonth = {};
  data.forEach(t => { if (!byMonth[t.month]) byMonth[t.month] = []; byMonth[t.month].push(t); });
  content.innerHTML = Object.keys(byMonth).sort().reverse().map(m => `
    <div class="section-block">
      ...
      <div class="tbl-wrap"><table class="dtbl">
        ...
        <tbody>${byMonth[m].map(t => `<tr>
          <td class="td-gold">${t.paramId}</td><td class="td-label">${t.param}</td>
          <td class="td-unit">${t.unit}</td>
          <td style="color:var(--maroon);font-weight:700;font-family:var(--mono)">${fmt(t.target,2)}</td>
          <td>${t.notes||''}</td><td style="color:var(--txt3);font-size:11px">${t.setBy||''}</td>
...
function openTargetModal() {
  ['tgt-month','tgt-param-id','tgt-param','tgt-unit','tgt-value','tgt-notes','tgt-rownum'].forEach(id => { document.getElementById(id).value = ''; });
...
async function saveTarget() {
  ...
  if (!month || !paramId) { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = 'Month and Param ID are required.'; return; }

  const result = await api('targets/save', { month, paramId, param, unit, target: isNaN(target) ? 0 : target, notes, rowNum: rowNum || null });
  if (result && result.success) { closeModal('target'); showToast('Target saved'); loadTargets(); }
  else { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = result ? result.error : 'Save failed'; }
}
...
async function loadReport() {
  ...
  if (data.crushing) {
    ...
    html += `<div class="report-card"><div class="rc-title" style="color:var(--crit)">⛏ Crushing</div><div class="rc-kpi-grid">
  ...
  if (data.milling) {
    ...
    html += `<div class="report-card"><div class="rc-title" style="color:#2471A3">⚙ Milling</div><div class="rc-kpi-grid">
  ...
  html += '</div>';
  if (data.chemicals && data.chemicals.some(c => c.total > 0)) {
    html += `<div class="report-card" style="margin-bottom:14px"><div class="rc-title">🧪 Chemical Consumption</div><div class="tbl-wrap"><table class="dtbl">
      <thead><tr><th>Chemical</th><th>Unit</th><th>Total</th></tr></thead>
      <tbody>${data.chemicals.filter(c => c.total > 0).map(c => `<tr><td class="td-label">${c.name}</td><td class="td-unit">${c.unit}</td><td class="td-val">${fmt(c.total,1)}</td></tr>`).join('')}</tbody>
    </table></div></div>`;
  }
  if (data.stoppages) {
    const s = data.stoppages;
    html += `<div class="report-card"><div class="rc-title">🛑 Stoppage Analysis — Total: ${fmt(s.total,1)} hrs</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-top:12px">
        <div><div style="font-size:11px;font-weight:700;color:var(--txt2);margin-bottom:8px;text-transform:uppercase">By Section</div>
          ${(s.bySection||[]).map(x=>`<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr);font-size:12px"><span style="color:var(--txt2)">${x.section}</span><span class="td-val">${fmt(x.hrs,1)} hrs</span></div>`).join('')||'<div class="nodata" style="padding:8px 0">No data</div>'}</div>
        <div><div style="font-size:11px;font-weight:700;color:var(--txt2);margin-bottom:8px;text-transform:uppercase">By Department</div>
          ${(s.byDept||[]).map(x=>`<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr);font-size:12px"><span style="color:var(--txt2)">${x.dept}</span><span class="td-val">${fmt(x.hrs,1)} hrs</span></div>`).join('')||'<div class="nodata" style="padding:8px 0">No data</div>'}</div>
        <div><div style="font-size:11px;font-weight:700;color:var(--txt2);margin-bottom:8px;text-transform:uppercase">Top Reasons</div>
          ${(s.topReasons||[]).slice(0,8).map((x,i)=>`<div style="padding:4px 0;border-bottom:1px solid var(--bdr);font-size:12px"><span style="color:var(--gold);font-weight:700">${i+1}.</span> <span style="color:var(--txt2)">${x.reason}</span><span class="td-val" style="float:right">${fmt(x.hrs,1)} hrs</span></div>`).join('')||'<div class="nodata" style="padding:8px 0">No data</div>'}</div>
      </div>
    </div>`;
  }
...
function initAdmin() { adminShowPanel(STATE.adminPanel || 'users'); }
...
function adminShowPanel(panel) {
  ...
  document.querySelectorAll('.admin-nav-item').forEach(b => {
    if (b.getAttribute('onclick') && b.getAttribute('onclick').includes("'" + panel + "'")) b.classList.add('active');
  });
...
async function loadAdminUsers() {
  ...
  if (!data || data.error) { content.innerHTML = `<div class="nodata">${data ? data.error : 'Error'}</div>`; return; }
  if (!data.length) { content.innerHTML = '<div class="nodata">No users found.</div>'; return; }
  content.innerHTML = `<div class="tbl-wrap"><table class="dtbl">
    ...
    <tbody>${data.map(u => `<tr>
      <td class="td-label">${u.username}</td><td>${u.name}</td>
      <td><span class="role-tag" style="background:var(--gold)">${u.role}</span></td>
      <td style="color:var(--txt3)">${u.email}</td>
      <td>${u.active==='true'||u.active==='1'||u.active==='yes' ? '<span style="color:var(--ok);font-weight:700">✔ Yes</span>' : '<span style="color:var(--crit);font-weight:700">✗ No</span>'}</td>
...
async function saveUser() {
  ...
  if (!username || !role) { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = 'Username and role required.'; return; }

  const result = await api('users/save', { username, name, role, email, password: password || undefined, active });
  if (result && result.success) { closeModal('user'); showToast('User saved'); loadAdminUsers(); }
  else { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = result ? result.error : 'Save failed'; }
}

let _limitsData = [], _limitsFilter = 'ALL';
...
function renderLimitsTable() {
  const content = document.getElementById('limits-content');
  const prefixes = ['ALL'];
  _limitsData.forEach(r => { const pf = String(r['ID']||'').split('_')[0]; if (pf && !prefixes.includes(pf)) prefixes.push(pf); });
  const filtered = _limitsFilter === 'ALL' ? _limitsData : _limitsData.filter(r => String(r['ID']||'').startsWith(_limitsFilter + '_'));

  content.innerHTML = `
    <div class="prefix-btns">${prefixes.map(p => `<button class="prefix-btn ${_limitsFilter===p?'active':''}" onclick="setLimitsFilter('${p}')">${p}</button>`).join('')}</div>
    <div class="tbl-wrap"><table class="dtbl limits-tbl">
      <thead><tr><th>ID</th><th>Label</th><th>Min (Crit)</th><th>Max (Crit)</th><th>Warn Min</th><th>Warn Max</th><th>Unit</th><th>Action</th></tr></thead>
      <tbody>${filtered.map(r => { const id = r['ID']||''; return `<tr>
        <td class="td-gold" style="white-space:nowrap">${id}</td><td class="td-label">${r['Label']||''}</td>
...
function setLimitsFilter(prefix) { _limitsFilter = prefix; renderLimitsTable(); }

async function saveLimitRow(id, rowNum) {
  const r = _limitsData.find(x => x['ID'] === id);
  if (!r) return;
  const getVal = sfx => { const el = document.getElementById(`lim_${sfx}_${id}`); return el ? (el.value !== '' ? parseFloat(el.value) : '') : ''; };
...
async function loadChemInventory() {
  ...
  if (!data || data.error) { content.innerHTML = `<div class="nodata">${data ? data.error : 'Error'}</div>`; return; }
  if (!data.length) { content.innerHTML = '<div class="nodata">No chemical inventory data found.</div>'; return; }
  const cols = Object.keys(data[0]).filter(k => k !== 'rowNum');
  content.innerHTML = `<div class="tbl-wrap"><table class="dtbl">
    <thead><tr>${cols.map(c=>`<th>${c}</th>`).join('')}</tr></thead>
    <tbody>${data.map(r=>`<tr>${cols.map(c=>`<td>${r[c]!==undefined?r[c]:''}</td>`).join('')}</tr>`).join('')}</tbody>
  </table></div>`;
}
...
async function doChangePassword() {
  ...
  if (!current || !newPw || !confirm) { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = 'All fields are required.'; return; }
  if (newPw !== confirm) { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = 'New passwords do not match.'; return; }
  if (newPw.length < 6) { msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = 'Password must be at least 6 characters.'; return; }

  const result = await api('change-password', { currentPassword: current, newPassword: newPw });
  if (result && result.success) {
    msg.className = 'form-msg success'; msg.style.display = 'block'; msg.textContent = 'Password changed successfully.';
    showToast('Password changed');
    ['pw-current','pw-new','pw-confirm'].forEach(id => { document.getElementById(id).value = ''; });
  } else {
    msg.className = 'form-msg error'; msg.style.display = 'block'; msg.textContent = result ? result.error : 'Change failed';
  }
}
