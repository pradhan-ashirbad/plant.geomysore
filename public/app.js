...
function renderDetailData(activeKey, data) {
  ...
  if (showProdTrend) {
    // Insert right after the LAST KPI row (a section can have more than one,
    // e.g. Filter Press) — or right after the sub-tabs/heatmap area if a
    // section has no KPI row — so it sits near the top, above
    // tables/stoppages, not sandwiched between KPI rows or buried at the bottom.
    const kpiRows = content.querySelectorAll('.kpi-row-1, .kpi-row-2, .kpi-row-3');
    const kpiRow = kpiRows.length ? kpiRows[kpiRows.length - 1] : null;
...
function renderFilterPress(data) {
  const rows = data.rows || [];
  let totalCycles = 0, totalCakeWt = 0, totalDryWt = 0, totalAuG = 0;
  const moistVals = [], auVals = [];
  rows.forEach(r => {
    totalCycles += parseFloat(r['Cycles'])  || 0;
    totalCakeWt += parseFloat(r['Cake Wt']) || 0;
    totalDryWt  += parseFloat(r['Dry Wt'])  || 0;
    totalAuG    += parseFloat(r['Au (g)'])  || 0;
    const m = parseFloat(r['Moisture']); if (!isNaN(m)) moistVals.push(m);
    const au = parseFloat(r['Au']);      if (!isNaN(au)) auVals.push(au);
  });
  ...
  return `
    <div class="kpi-row-3">
      <div class="kpi-tile">
        ...
        <div class="kpi-val">${fmt(totalCycles,0)}</div>
        <div class="kpi-unit">cycles</div>
      </div>
      <div class="kpi-tile">
        <div class="kpi-label">Cake Wt</div>
        <div class="kpi-val">${fmt(totalCakeWt,1)}</div>
        <div class="kpi-unit">t</div>
      </div>
      <div class="kpi-tile">
        <div class="kpi-label">Dry Wt</div>
        <div class="kpi-val gold">${fmt(totalDryWt,1)}</div>
        <div class="kpi-unit">t</div>
      </div>
    </div>
    <div class="kpi-row-3">
      <div class="kpi-tile">
        ...
        <div class="kpi-val">${fmt(avg(moistVals),1)}</div>
        <div class="kpi-unit">%</div>
      </div>
      <div class="kpi-tile">
        <div class="kpi-label">Avg Au</div>
        <div class="kpi-val">${fmt(avg(auVals),3)}</div>
        <div class="kpi-unit">ppm</div>
      </div>
      <div class="kpi-tile">
        <div class="kpi-label">Au Content</div>
        <div class="kpi-val gold">${fmt(totalAuG,1)}</div>
...
function loadTrendSeries(sectionKey, data) {
  try {
    ...
    if (raw) {
      ...
      // Drop any saved series whose parameter no longer exists in the current
      // schema (e.g. a section whose columns were redefined) — otherwise the
      // chart renders blank and the customizer shows empty rows. If every
      // saved series is stale, fall through to the default below.
      if (Array.isArray(saved) && data) {
        const validKeys = new Set(_chartableParams(data).map(p => p.key));
        const pruned = saved.filter(s => validKeys.has(s.paramKey));
        if (pruned.length !== saved.length) saveTrendSeries(sectionKey, pruned);
        saved = pruned;
      }
